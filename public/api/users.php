...
function get_authenticated_user($conn) {
    ...
    if (empty($token)) {
        ...
        echo json_encode(["error" => "Authentication required", "logout" => true]);
        exit;
    }
    ...
    if (count($parts) >= 1) {
        ...
        if (is_array($decoded) && !empty($decoded['user'])) {
            ...
            try {
                ...
                http_response_code(401);
                echo json_encode(["error" => "Session validation error", "logout" => true]);
                exit;
            }
        }
    }
    http_response_code(401);
    echo json_encode(["error" => "Invalid authentication token", "logout" => true]);
    exit;
}
